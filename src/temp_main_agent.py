...
def _infer_complexity(
    ...
    """
    Heuristic complexity calculator used when the LLM / tools do not
    return a reasonable complexity_score.
    """
...
def run_full_pipeline(
    ...
    complexity_block = risk_output.get("complexity") or {}
    ...
    # Ensure complexity is populated
    sim_complexity = simulation_output.get("complexity") or {}
    score = sim_complexity.get("complexity_score") or 0
    if not score:
        sim_complexity = _infer_complexity(requirements, raid_log)
        simulation_output["complexity"] = sim_complexity

    baseline_timeline_weeks = (
        simulation_output.get("baseline_timeline_weeks") or deadline_weeks
    )
